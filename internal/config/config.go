...
type Config struct {
	...
	// StorageDriver selects the persistence backend: "auto" (default), "memory" (Zero-Docker mode), or "postgres".
...
func Load() (Config, error) {
	...
	cfg := Config{
		...
		StorageDriver:                         envOr("TELESRV_STORAGE_DRIVER", envOr("STORAGE_DRIVER", "auto")),
...
func readEnvFile(path string) (envSource, error) {
	...
	for lineNo := 1; scanner.Scan(); lineNo++ {
		...
		if !strings.HasPrefix(key, "TELESRV_") {
			// Skip application keys belonging to the client or other tools
			continue
		}
		if !validEnvKey(key) {
